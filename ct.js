...
const ct = {
     ...
     */
    version: '1.5.1',
    meta: [{"name":"RogeLike","author":"Atndesign","site":"","version":"0.0.0"}][0],
    main: {
        fpstick: 0,
        pi: 0
    },
...
ct.inputs.addAction('moveUp', [{"code":"keyboard.ArrowUp","multiplier":-1}]);
ct.inputs.addAction('moveDown', [{"code":"keyboard.ArrowDown"}]);
ct.inputs.addAction('moveLeft', [{"code":"keyboard.ArrowLeft","multiplier":-1}]);
...
var inGameRoomStart = function (room) {
    room.chest = 0;
    room.chestTotal = ct.types.list['chest'].length;
...
ct.rooms.templates['Room_3c28ba730325'] = {
    name: 'Room_3c28ba730325',
    width: 1920,
    height: 1080,
    /* JSON.parse allows for a much faster loading of big objects */
    objects: JSON.parse('[{"x":192,"y":256,"exts":{},"type":"player"},{"x":128,"y":192,"exts":{},"type":"torch"}]'),
    bgs: JSON.parse('[]'),
...
for (var x = 0; x < ct.camera.width / 64; x++) {
    for (var y = 0; y < ct.camera.height / 64; y++) {
        ...
        if (value < - 0.5) {
            ...
            tilemap.addTile('floor_2', x*64, y*64);
            // Tiles are PIXI.Sprites; we can tweak their color and opacity before caching
            }
        else if (value < 0.3) {
            tilemap.addTile('floor_3', x*64, y*64);
            // Tiles are PIXI.Sprites; we can tweak their color and opacity before caching
            }
        else if (value < 0.4) {
            tilemap.addTile('floor_4', x*64, y*64);
            // Tiles are PIXI.Sprites; we can tweak their color and opacity before caching
            }
        else if (value < 0.5) {
            tilemap.addTile('floor_5', x*64, y*64);
            // Tiles are PIXI.Sprites; we can tweak their color and opacity before caching

        }
        else if (value < 0.6) {
            tilemap.addTile('floor_6', x*64, y*64);
            // Tiles are PIXI.Sprites; we can tweak their color and opacity before caching
            }
    }
}
...
tilemap.cache();
const randomNbrOfChest = Math.floor(Math.random() * 6 + 1)
for(let i = 0; i < randomNbrOfChest; i++){
    ct.types.copy('chest', Math.floor(Math.random() * this.width), Math.floor(Math.random() * this.height))
}
...
ct.rooms.templates['LayerUI'] = {
    ...
    height: 1080,
    /* JSON.parse allows for a much faster loading of big objects */
    objects: JSON.parse('[{"x":64,"y":128,"exts":{},"type":"chestCounter"}]'),
    bgs: JSON.parse('[]'),
    tiles: JSON.parse('[{"depth":-10,"tiles":[],"extends":{}}]'),
    backgroundColor: '#000000',
    onStep() {
...
ct.camera.scale.x = 0.3
ct.camera.scale.y = 0.3
this.speed = 2 * ct.delta;

if(ct.actions.moveUp.down){
    this.vspeed = -this.speed * 2
    if (this.tex !== 'player_run') {
        this.tex = 'player_run';
        this.play();
    }
}
...
else if(ct.actions.moveLeft.down){
    this.hspeed = -this.speed * 2;
    if (this.tex !== 'player_run') {
        this.tex = 'player_run';
        this.play();
    }
...
ct.types.list['chest'] = [];
ct.types.templates["chestCounter"] = {
    depth: 0,
    texture: "New_Piskel",
    onStep: function () {
        this.move();
    ...
    onDraw: function () {
        this.text.text = `${ct.room.chest} / ${ct.room.chestTotal}`;
    },
    onDestroy: function () {
        
    },
    onCreate: function () {
        this.text = new PIXI.Text('0 / ' + ct.room.chestTotal, ct.styles.get('ChestCounter'));
this.text.x = 32;
